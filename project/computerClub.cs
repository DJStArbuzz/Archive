...
namespace Game
{
    ...
    class ComputerClub
    {
        ...
        public void Work()
        {
            while (_clients.Count > 0)
            {
                ...
                Console.WriteLine($"Баланс компьютерного клуба {_money}. " +
                    $"Ждем нового клиента.");
                ...
                if(int.TryParse(userInput, out int computerNumber))
                {
                    ...
                    if (computerNumber >= 0 && computerNumber < _computers.Count)
                    {
                        ...
                        {
                            Console.WriteLine("Вы пытаетесь посадить клиента за тот компьютер, который занят. " +
                        ...
                        else
                        {
                            ...
                            {
                                Console.WriteLine("Клиент оплатил деньги и сел за компьютер " + computerNumber + 1);
                                _money += newClient.Pay();
                                _computers[computerNumber].BecomeTaken(newClient);
                            }
                            else
                            {
                                Console.WriteLine("У клиента не хватило денег, а потому он ушел домой.");
                            }
                        }
                ...
                Console.WriteLine("Нажмите любую клавишу, чтобы перейти к новому клиенту. ");
                Console.ReadKey();
                Console.Clear();
                SpendOneMinute();
            }
    ...
    class Computer
    {
        ...
        public Computer(int priceMinute)
        {
